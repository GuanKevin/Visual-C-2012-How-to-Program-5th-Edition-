using System;

/**
 * 7.17 (Even or Odd) 
 * Write method IsEven that uses the remainder operator (%) to determine whether an integer is even. The method should 
 * take an integer argument and return true if the integer is even and false otherwise. Incorporate this method into an 
 * app that inputs a sequence of integers (one at a time) and determines whether each is even or odd. Incorporate this 
 * method into an app that reads an integer value for side from the user and outputs the asterisks with the 
 * SquareOfAsterisks method.
 * ****
 * **** 
 * ****
 * ****
 */
namespace CHP07PE18
{
    class Program
    {
        static void Main(string[] args)
        {
        }
    }
}
