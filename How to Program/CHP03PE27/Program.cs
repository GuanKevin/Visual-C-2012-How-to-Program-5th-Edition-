 ...
 * integer representations for those characters is called that computer’s character set. You can indicate a character value in an app simply
 * by enclosing that character in single quotes, as in 'A'. You can determine the integer equivalent of a character by preceding that 
 * character with (int), as in (int) 'A' The keyword int in parentheses is known as a cast operator, and the entire expression is called a 
 * cast expression. The following statement outputs a character and its integer equivalent: 
